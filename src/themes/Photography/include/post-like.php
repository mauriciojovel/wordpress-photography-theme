...
function sl_enqueue_scripts() {
    wp_enqueue_script( 'simple-likes-public-js', get_template_directory_uri() . '/assets/js/simple-likes-public.js', array( 'jquery' ), 
	...
	if ( $post_id != '' ) {
		...
		if ( !already_liked( $post_id, $is_comment ) ) { // Like the post
			if ( is_user_logged_in() ) { // user is logged in
				...
				// 	}
                // }
                wp_send_json_error(['errorCode'=>403, 'errorMessage'=> __('You must be logged to vote', 'photography')], 403);
                die;
			}
			$like_count = ++$count;
