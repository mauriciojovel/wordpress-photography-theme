    ...
    <?php if( ( in_array( 'judge', (array) $user->roles ) || in_array( 'photographer', (array) $user->roles ) ) && count($usercomment) == 0 ) : ?>
        <?php
            $comments_arg = array(
                ...
                'title_reply' => __('Leave your comment', 'photography'),
                'logged_in_as' => '',
                'reply_text' => '',
                'fields' => apply_filters( 'comment_form_default_fields', array(
                        'autor' 				=> '<div class="form-group">' . '<label for="author">' . __( 'Name', 'photography' ) . '</label>' .
                                                '<input id="author" name="author" class="form-control" type="text" value="" size="30" />'.
                                                '<p id="d1" class="text-danger"></p>' . '</div>',
                        'email'					=> '<div class="form-group">' .'<label for="email">' . __( 'Email', 'photography' ) . '</label> ' .
                                                '<input id="email" name="email" class="form-control" type="text" value="" size="30" />'.
                                                '<p id="d2" class="text-danger"></p>' . '</div>',
                        'url'					=> '')),
                        'comment_field'			=> '<div class="form-group">' . '<label for="comment">' . __( 'Comment', 'photography' ) . '</label><span>*</span>' .
